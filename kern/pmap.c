...
page_init(void)
{
	...
	size_t num_hole = 96; //384KB/4KB
	for(i = 0; i<npages; i++){
		if(i == 0){
			pages[i].pp_ref = 1;
			pages[i].pp_link = NULL;
		}
		else if(i >= npages_basemem && i < npages_basemem + num_hole + num_alloc){
			pages[i].pp_ref = 1;
			pages[i].pp_link = NULL;
		}
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	pde_add = pgdir + pd_index;
	if(!(*pde_add & PTE_P)){
		if(create){
			if(!(new_page = page_alloc(1)))
				return NULL;
			new_page->pp_ref +=1;
			*pde_add = page2pa(new_page) | PTE_P | PTE_W | PTE_U;
		}
		else
			return NULL;
	}
		pgtb = KADDR(PTE_ADDR(*pde_add));
		uint32_t pt_index = PTX(va);
		return &pgtb[pt_index];
}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	pte_t *entry;
	char *st;
	for(st = ROUNDDOWN((char*)va,PGSIZE); st < ROUNDUP((char*)(va+len),PGSIZE); st += PGSIZE){
		entry = pgdir_walk(env->env_pgdir, st, 0);
		if((uintptr_t)st >= ULIM || entry == NULL || ((*entry & (perm| PTE_P)) != (perm | PTE_P))){
			if(st == ROUNDDOWN((char*)va,PGSIZE))
